#!/usr/bin/env python3
"""Check deployable files and links at either a domain root or a project subpath."""
...
def check_xml(root, base_url):
    failures = []
    namespace = {"a": "http://www.w3.org/2005/Atom", "s": "http://www.sitemaps.org/schemas/sitemap/0.9"}
...
def check(root, base_url="https://example.invalid"):
    ...
    required = (
        "index.html", "about/index.html", "software/index.html", "writing/index.html",
        "404.html", "atom.xml", "sitemap.xml", "giallo-light.css", "giallo-dark.css",
        "js/theme.js", "js/pendulum.js",
    )
    ...
    forbidden = ["prototype-controls", "What becomes possible after the trait solver?", "/prototype/", "content/prototype", "data-variant", "data-design"]
    ...
    if failures:
        sys.exit("\n".join(failures))
    print(f"Output checks passed: {len(documents)} pages, local links, canonical/feed URLs, allowed scripts and no prototype content.")


if __name__ == "__main__":
    check(Path(sys.argv[1]), sys.argv[2] if len(sys.argv) > 2 else "https://example.invalid")
