...
def check_articles(zola):
    ...
    with tempfile.TemporaryDirectory(prefix="personal-site-check-") as temporary:
        ...
        shutil.copy(
            source / "content/writing/_index.md", root / "content/writing/_index.md"
        )
...
""")
        ...
        assert len(entries) == 1, (
            "Only the published test article should be in the feed"
        )
        ...
        for path in (root / "public").rglob("*"):
            if path.is_file() and path.suffix in (".xml", ".html"):
                assert "draft-test-sentinel" not in path.read_text(), (
                    f"Draft leaked into {path}"
                )
                assert "draft-only-tag-sentinel" not in path.read_text(), (
                    f"Draft tag leaked into {path}"
                )
...
""")
        ...
        for base_url in ("https://site.example", "https://site.example/blog"):
            subprocess.run(
                [str(zola), "-r", str(root), "build", "--base-url", base_url],
                check=True,
            )
        ...
        home_path.write_text(
            home_path.read_text().replace(
                "/blog/css/layout.css", "/blog/css/missing.css"
            )
        )
        try:
    ...
    print(
        "Content checks passed: feeds, draft/tag exclusion, filtered tag archives and homepage limits."
    )
